function [RhoMap, RhoTru, epsMap, LiniMap, Locs, CumuSet, Wset] = xy_getPSF_paraEsti_eps_rho_N(xdim,ydim,mvlength,inputPath,inputName,outputPath,ImMeanLoc,sig,saveOption,tauSeries, AcceptFactor, UestiFactor, IniFactor, FineGridFold);
...
if sum(tauSeries) == 0
    ...
    for ord = 2:7; eval(['CumuSet.AC',num2str(ord),'=AC',num2str(ord),';']);end;
else
...
[LiniMap, Locs] = xy_esti_LMap(epsMap, RhoMap, AC7, AcceptFactor, UestiFactor, IniFactor, sig, FineGridFold);
p = RhoMap;
Wset.w2 = p -     p.^2;
Wset.w3 = p -  3.*p.^2 +   2.*p.^3;
Wset.w4 = p -  7.*p.^2 +  12.*p.^3 -    6.*p.^4;
Wset.w5 = p - 15.*p.^2 +  50.*p.^3 -   60.*p.^4 +   24.*p.^5;
Wset.w6 = p - 31.*p.^2 + 180.*p.^3 -  390.*p.^4 +  360.*p.^5 -  120.*p.^6;
Wset.w7 = p - 63.*p.^2 + 602.*p.^3 - 2100.*p.^4 + 3360.*p.^5 - 2520.*p.^6 + 720.*p.^7;
